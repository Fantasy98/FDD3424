...
fileID  = Name_Costum_VAE(args, Ntrain)
...
det_id      = 'det_'+ fileID
det_path    =  base_dir + "04_Figs/CorrMatrix/"+det_id
...
det_id      = 'Mode_'   + fileID
det_path    = base_dir  + "04_Figs/SpModes/"+det_id
...
np.savez_compressed(base_dir + "03_Mode/"+fileID+"modes.npz",
                        z_mean      = Z_mean,
                        z_var       = Z_var,
                        modes       = Modes,
...
snap_path   = base_dir  + "04_Figs/PredSnap/"+snap_id
...
snap_path   = "04_Figs/loss/"+snap_id
...
new_data = [{"Type":model_type,
            "Epoch":epochs,
            "beta":beta,
            "latent_dim":latent_dim,
            "Nfield": Ntrain,
            "tot_loss":np.round(history["tot_loss"][-1],4),
            "rec_loss":np.round(history["rec_loss"][-1],4),
            "kl_loss":np.round(history["kl_loss"][-1],4),
            "E_k" :np.round(Energy_rec,4),
            "R" :np.round(detR,4) ,
            "time": np.round(ckpt['time']),
            }]
...
print("INFO: The results have been written in csv")
