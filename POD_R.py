...
if args.full: 
    r         =   -1
    print(f"INFO: SVD complete, Going to use FULL modes for reconstruction")

else: 
    r         =   args.r
    print(f"INFO: SVD complete, Going to use {r} modes for reconstruction")
...
u_p        *=   cov
u          *=   cov
u           =   u.T.reshape((-1, nx, ny,nc))
u_p         =   u_p.T.reshape((-1, nx, ny,nc))
...
np.savez_compressed( base_dir + podfile +f"POD-m{r}-n{nt}"+".npz",
                    u_p   = u_p,
                    modes = U,
                    vh    = vh,
                    s     = s
