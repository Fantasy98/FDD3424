...
Verison 2 
We use two Predictors for mu and sigma in latent space
...
class VAE_ROM(nn.Module):
    def __init__(self, 
        ...
        if self.ROM_config.predictor_model == "TFSelf":
            ...
            self.Predictor_Mean  =  EmbedTransformerEncoder(
            ...
            self.Predictor_Var  =  EmbedTransformerEncoder(
                                                        d_input     = self.predictor_config.in_dim,
                                                        d_output    = self.predictor_config.next_step,
                                                        n_mode      = self.predictor_config.nmode,
                                                        d_proj      = self.predictor_config.time_proj,
        ...
        if self.ROM_config.predictor_model == "TFEasy":
            ...
            self.Predictor_Mean  = easyTransformerEncoder(
                                                        d_input     =   self.predictor_config.in_dim,
                                                        d_output    =   self.predictor_config.next_step,
            ...
            self.Predictor_Var  = easyTransformerEncoder(
                                                        d_input     =   self.predictor_config.in_dim,
                                                        d_output    =   self.predictor_config.next_step,
                                                        seqLen      =   self.predictor_config.nmode,
                                                        d_proj      =   self.predictor_config.time_proj,
    ...
    def Complie(self, 
        ...
        # Optimizer                     
        self.optimizer          =   Adam([
                                            {'params': self.Predictor_Mean.parameters()},
                                            {'params': self.Predictor_Var.parameters()}
                                        ],         
    ...
    def Solve_ROM(self):
        ...
        self.Predictor_Mean.to(device)
        self.Predictor_Var.to(device)
        ...
        for epoch in range(epochs):
            ...
            self.Predictor_Mean.to(self.device)
            self.Predictor_Var.to(self.device)
            self.VAE.to(self.device)
            self.Predictor_Mean.train()
            self.Predictor_Var.train()
            for x, y in (self.train_dl):
                ...
                src_z_mean           =   torch.cat(
                                                        [self.VAE.encoder(x[:,t,:,:])[0].view(B,1,self.Z) for t in range(T)], 
                                                      
                                                      dim = 1).to(self.device).float()
                src_z_var           =   torch.cat(
                                                        [self.VAE.encoder(x[:,t,:,:])[1].view(B,1,self.Z) for t in range(T)], 
                ...
                # Make prediction
                pre_z_mean            =   self.Predictor_Mean(src_z_mean)
                pre_z_var             =   self.Predictor_Var(src_z_var)


                tgt_z_mean           =   torch.cat(
                                                        [self.VAE.encoder(y[:,n,:,:])[0].view(B,1,self.Z) for n in range(N)], 
                                                      
                                                      dim = 1).to(self.device).float()
                tgt_z_var           =   torch.cat(
                                                        [self.VAE.encoder(y[:,n,:,:])[1].view(B,1,self.Z) for n in range(N)], 
                                                      
                                                      dim = 1).to(self.device).float()
                
                latent_loss_            =   self.latent_loss(pre_z_mean, tgt_z_mean) + self.latent_loss(pre_z_var, tgt_z_var)
                
                


                pre_z_vector            =   torch.cat([self.VAE.reparameterize((pre_z_mean[:,n,:], pre_z_var[:,n,:])).view(B,1,self.Z) for n in range(N)],
                                                      dim = 1)
            ...
            if self.val_dl != None:
                ...
                self.Predictor_Mean.eval()
                self.Predictor_Var.eval()
                ...
                for x,y in (self.val_dl):
                    ...
                    val_latent_loss_            =   self.latent_loss(pre_z_mean, tgt_z_mean) + self.latent_loss(pre_z_var, tgt_z_var)
                    ...
                    val_tot_loss                =   latent_loss_    * self.latent_loss_weight +\
                                                 rec_loss_       * self.rec_loss_weight
    ...
    def Save_checkpoint(self, base_dir):
        ...
        save_checkpoint_to  =   base_dir + "06_ROM/VAEPredictor/CheckPoints/" + "R2_" + case_name
        ...
        if self.ROM_config.save_predictor_only: 
            ...
            ckPoint             =   {
                                        "model_mean"    :   self.Predictor_Mean.state_dict(), 
                                        "model_var"     :   self.Predictor_Var.state_dict(), 
    ...
    def Load_ROM_state(self, base_dir):
        ...
        case_name           =   Name_ROM(self.ROM_config)
        load_checkpoint_as  =   base_dir + "06_ROM/VAEPredictor/CheckPoints/" + "R2_"+ case_name + ".pt"
    ...
    def GenPredicton(self, test_data):
        ...
        for x, _ in (dl):
            ...
            Latent_mean_ref.append(z_mean.detach().cpu().numpy()) 
            Latent_var_ref.append(z_var.detach().cpu().numpy()) 
        
        del dl, x, z_mean, z_var

        print(f"INFO: Generate the reference latent space variables")

        Latent_mean_ref             =   np.concatenate(Latent_mean_ref,0)
        Latent_var_ref              =   np.concatenate(Latent_var_ref,0)
        print(f"The generated latent variables are:\n"+\
              f" MEAN =  {Latent_mean_ref.shape}, VAR = {Latent_var_ref.shape}")
        Latent_mean_predictions     =   deepcopy(Latent_mean_ref)
        Latent_var_predictions      =   deepcopy(Latent_var_ref)
        Physic_predictions          =   []
        
        for i  in  range(seqLen, Nt - next_step):

            src_mean        =   Latent_mean_ref[None, i-seqLen:i, :]
            src_var         =   Latent_var_ref[None, i-seqLen:i, :]

            src_mean        =   torch.from_numpy(src_mean).float().to(self.device)
            src_var         =   torch.from_numpy(src_var).float().to(self.device)
          
            pred_mean       =   self.Predictor_Mean(src_mean)   
            pred_var        =   self.Predictor_Var(src_var)   

            pred_vector     =   torch.cat(
                                            [self.VAE.reparameterize((pred_mean[:,nxstp,:],pred_var[:,nxstp,:] )) for nxstp in range(next_step)],
                                            dim = 1
                                            )

            snap            =   self.VAE.decoder(pred_vector)
          

            Latent_mean_predictions[i:i+next_step, :]     = pred_mean.detach().cpu().numpy()
            Latent_var_predictions[i:i+next_step, :]      = pred_var.detach().cpu().numpy()
            
            Physic_predictions.append(snap.detach().cpu().numpy())

        Physic_predictions  =   np.concatenate(Physic_predictions, 0)
        Latent_predictions  =   np.concatenate([Latent_mean_predictions,Latent_var_predictions],0)
