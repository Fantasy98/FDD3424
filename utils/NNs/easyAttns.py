...
class WeasyTransformerEncoder(nn.Module):
    """
    A transformer-based architecture using temporal-spatial embedding and a stack of encoder 
    ...
    def __init__(self, d_input, d_output, seqLen, 
                        ...
                        act_proj= "relu",
                        dropout= 1e-5) -> None:
        super(WeasyTransformerEncoder,self).__init__()
        ...
        self.weight_sums =[           
                                            nn.Parameter(torch.randn(size =(d_proj,d_model),dtype=torch.float32), requires_grad=True) for _ in range(num_layer)
                                            ]
    ...
    def forward(self, src):
        ...
        for i, enc_layer in enumerate(self.encoders):
            enc_input_  = enc_input

            enc_input   = enc_layer(enc_input * self.act_weight( self.weight_sums[i]) + enc_input_)
