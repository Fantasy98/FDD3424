...
def Sliding_Window_Mean_Ek(
    ...
    def E_k(predictions, targets):
        return (1 -  np.mean( (predictions - targets) ** 2) / np.mean(targets**2) ) *100
    ...
    SeqLen = test_mean.shape[0]
    error = None

    test_data_space *=  u_std
    test_var = torch.from_numpy(test_var).float().to(device)

    for init in tqdm(range(in_dim,SeqLen-window,5)):
        temporalModes_pred = copy.deepcopy(test_mean)
        spatialSnap_pred   = copy.deepcopy(test_data_space)
        for timestep in range(init, init+window):

            data    = temporalModes_pred[None, (timestep-in_dim):timestep, :]
            feature = torch.from_numpy(data).float().to(device)
            mean_pred    = model(feature)
            var_tgt      = test_var[None, timestep:timestep + 1,:]
  
            pred         = mean_pred + torch.exp(0.5*var_tgt) * torch.rand_like(var_tgt).float().to(device)

            spatialSnap_pred[timestep,:,:,:] = decoder(pred).detach().cpu().numpy()

        if error is None:
            error   = E_k(spatialSnap_pred[init:init+window,:,:,:] * u_std , test_data_space[init:init+window,:,:,:])
            n = 1
        else:
            error   = E_k(spatialSnap_pred[init:init+window,:,:,:], test_data_space[init:init+window,:,:,:]) +  error 
            n += 1
