...
class TransformerEncoderBlock(Module):
    def __init__(self,  d_input, d_model, 
        ...
        ####################################
        if embed == "sin":
            self.embed = SineActivation(d_input,nmode,d_model)
        if embed == "cos":
            self.embed = CosineActivation(d_input,nmode,d_model)
        if embed == "posenc":
            self.embed = PosEncoding(nmode,d_input,d_model)
...
class Transformer2(nn.Module):
    def __init__(self,  d_input, d_model, 
        ...
        if is_output:
            ...
            self.out.add_module("out_1",nn.Conv1d(nmode,nmode,kernel_size=1))
            self.out.add_module("out_2",nn.Linear(d_model,out_dim))
