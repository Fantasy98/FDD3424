...
parser.add_argument("--compare","-c",action="store_true",help="VAE VS AE")
...
if Args.compare:
    print("#"*30)
    print("Plotting the PDF AE VS VAE:")
    ...
    for j, ax in enumerate(axs):
        ...
        if betas[j] == 0.0: 
            ...
            ax.set_title( "Arch 4: " + r"$\beta$ "+f"= {np.round(betas[j]*100,1)}" + r"$\times$ " + r"$10^2$" )
        print(f"For beta =  {betas[j]}, there are {n_noise} noises")
    axs[0].set_ylabel("PDF")
        

    plt.savefig(save_pdf_to + "AE_VAE",
                bbox_inches = "tight",
                dpi=300)
